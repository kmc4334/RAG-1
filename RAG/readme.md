...
### 방법 2: 대량 데이터 입력 (Bulk Ingestion)
`backend/ingest.py` 스크립트를 사용하여 JSON 파일에 있는 데이터를 한 번에 저장할 수 있습니다.

1. `backend/data.json` 파일에 아래 형식으로 데이터를 준비합니다.
   ```json
   {
       "documents": [
           {"url": "...", "text": "..."},
